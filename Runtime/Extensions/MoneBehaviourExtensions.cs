...
namespace Packages.Estenis.UnityExts_ {
  public static class MonoBehaviourExtensions {
    ...
    public static Coroutine RunCoroutineOnceAfterUnscaled(this MonoBehaviour monoBehaviour, Action action, float seconds) {
      if (monoBehaviour == null || !monoBehaviour.gameObject.activeSelf) {
        Debug.LogWarning($"{monoBehaviour?.gameObject.name ?? ""}.{nameof(RunCoroutineOnceAfter)} called when GameObject is not active.");
        return null;
      }
      return monoBehaviour.StartCoroutine(CoroutineHelpers.ExecuteOnceAfterUnscaled(action, seconds));
    }
